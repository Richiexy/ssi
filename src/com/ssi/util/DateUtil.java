...
public class DateUtil {
	...
	private static final FastDateFormat yyyyMMdd = FastDateFormat.getInstance("yyyyMMdd");//"yyyyMMdd"    日期格式化类型
	private static final FastDateFormat showDate = FastDateFormat.getInstance("yyyy年MM月dd日");//"yyyy年MM月dd日" 日期格式化类型
	...
	public static String formatStringToDate(String strFormat,String tranFormat, String strDate){
		...
		try {
			...
			throw new RuntimeException("字符转成时间格式出错。");
		}
	}
	 ...
	 * @param value
	 * @return 
	 * @date:2014-10-28 上午09:17:36
	 */
	public static Date dateChangeByMonth(Date date, int value) {
		GregorianCalendar cal = new GregorianCalendar();
		cal.setTime(date);
		cal.add(GregorianCalendar.MONTH, value);
		return cal.getTime();
	}
	...
	/**
	 * 纯粹以日期为计算基准,
	 * 如       2014-10-01 23:59:59 与 2014-10-02 00:00:01虽然隔两秒但是也是隔天
	 * @param start
	 * @param end
	 * @return
	 */
	public static int dataDiffByDays(Date start, Date end) {
		String startStr = dateToString(start);
		String endStr = dateToString(end);
		//重新格式化,忽略时分秒,只取天
		start = stringToDateShort(startStr);
		end = stringToDateShort(endStr);
		long startTime = start.getTime();
		long endTime = end.getTime();
		return (int)((endTime-startTime)/(24*60*60*1000));
	}
...
//					DateUtil.convertStringToDate("yyyy-MM-dd","2014-01-02"));
//		} catch (ParseException e) {
...
//		}
	 ...
	 * @param end
	 */
	public static int getDayNum(String start, String end) {
		int dataDiff = 0;
		try {
			dataDiff = DateUtil.dataDiff(DateUtil.convertStringToDate("yyyy-MM-dd",start),
					DateUtil.convertStringToDate("yyyy-MM-dd",end));
		} catch (ParseException e) {
			e.printStackTrace();
		}
	...
	public static boolean isTrue(String str1,String str2){
		...
		if(DateUtil.convertMyStringtoDate("yyyy-MM-dd",str1).getTime() >= DateUtil.convertMyStringtoDate("yyyy-MM-dd",str2).getTime()){
			returnval = true;
		}
		return returnval;
	}
